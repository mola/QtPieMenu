...
{
  QtPieAction *action = new QtPieAction(icon, QString(), receiver, member);
...
{
  QPainter         p(&pix);
  QRadialGradient  gradientBack(rect().center().x(), rect().center().y() - 10, rect().width() / 2.0); // diagonal gradient from top-left to bottom-right

  gradientBack.setColorAt(0, QColor(174, 184, 191));
  gradientBack.setColorAt(0.25, QColor(220, 220, 220));
  gradientBack.setColorAt(0.85, QColor(239, 238, 239));
  gradientBack.setColorAt(1, QColor(190, 190, 190));

  QRadialGradient  gradientLight(rect().center().x(), rect().center().y() - 10, rect().width() / 2.0); // diagonal gradient from top-left to bottom-right
...
// p.setBrush(QBrush(palette().highlight()));
        p.setBrush(gradientLight);
      }
      else
      {
// p.setBrush(QBrush(palette().background()));
        p.setBrush(gradientBack);
      }
